...
static int current_battery_pencenty;
...
static void update_battery_percent(int percent)
{
	if(percent < 0 || percent > 100){
		LOGE("gezi_charger update_battery_percent error....\n");
		return;
	}
	LOGE("gezi_charger update_battery_percent = ============%d\n",percent);
	if(percent != current_battery_pencenty){
		current_battery_pencenty = percent;
	}
}


static void draw_battery_percent(int x_point,int y_point)
{
	int ret = 0;
	//int char_width_test = 50;
	//int char_height_test = 60;
	char percent[20];
	...
	sprintf(percent,"%d",current_battery_pencenty);
	
	if(current_battery_pencenty < 10)
		sprintf(&percent[1],"%c",'%');
	else if(current_battery_pencenty == 100)
		sprintf(&percent[3],"%c",'%');
	else
		sprintf(&percent[2],"%c",'%');
	...
	x = (gr_fb_width() - gr_measure(percent)) / 2;
	y = gr_fb_height() / 2  + 30;
	//gr_set_font_size(18, 32);
	#ifdef SUPPORT_WISKY_XDS
		ret = draw_text(percent, x, y+100);
	#else
		#ifdef BOARD_HAS_FLIPPED_SCREEN
			ret = draw_text(percent, x, y_point+50);
		#else
			ret = draw_text(percent, x, y_point-250);
		#endif 
	#endif 
	
	LOGE("gezi_charger draw_batterypercent x = %d\n",x);
}
...
static void draw_battery(struct charger *charger)
{
    ...
    if (batt_anim->num_frames != 0) {
        ...
        LOGE("gezi_charger drawing frame #%d min_cap=%d time=%d\n",
             batt_anim->cur_frame, frame->min_capacity,
             frame->disp_time);
		draw_battery_percent(750,1180);
    }
	else{
		LOGE("gezi_charger batt_anim->num_frames == 0...\n");
	}
}
...
static void redraw_screen(struct charger *charger)
{
    ...
    if (batt_anim->capacity < 0 || batt_anim->num_frames == 0){
		...
		LOGE("gezi_charger redraw_screen  draw_unknown..\n");
	}
    else{
        draw_battery(charger);
		LOGE("gezi_charger redraw_screen  draw_battery..\n");
	}
...
static void update_screen_state(struct charger *charger, int64_t now)
{
	...
	//int char_width_test = 20;
	//int char_height_test = 36;

    if (!batt_anim->run || now < charger->next_screen_transition){
		LOGE(" gezi_charger batt_anim->run || now < charger->next_screen_transition..return\n");
        return;
	}
	LOGE("gezi_charger 0...\n");
    if (!minui_inited) {
		...
		LOGE(" gezi_charger gr_font_size char_width = %d,char_height = %d\n",char_width,char_height);
    ...
    if (batt_anim->cur_frame == 0) {
		...
		update_battery_percent(batt_prop->batteryLevel);
		LOGE("gezi_charger batt_prop->batteryLevel===============0============ %d\n",batt_prop->batteryLevel);
        if (batt_prop && batt_prop->batteryLevel >= 0 && batt_anim->num_frames != 0) {
            int i;
		LOGE("gezi_charger batt_prop->batteryLevel=========================== %d\n",batt_prop->batteryLevel);
		...
		LOGE("gezi_charger batt_anim->capacity===========0=============== = %d\n",batt_anim->capacity);
        if (batt_prop){
            batt_anim->capacity = batt_prop->batteryLevel;
			LOGE("gezi_charger batt_anim->capacity========================== = %d\n",batt_anim->capacity);
		}
    }
