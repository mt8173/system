...
#define BTA_AV_RC_DISC_TIME_VAL_SPECIAL_DEVICE    10000
#endif
...
void bta_av_do_disc_a2d (tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    {
        p_scb->wait &= ~BTA_AV_WAIT_CHECK_RC;
        if (interop_match_addr(INTEROP_DISCOVER_TIME_OUT, (const bt_bdaddr_t *)&p_scb->peer_addr) == TRUE) {
            APPL_TRACE_DEBUG("Special Device, make the disvoer timer longer:10s");
            bta_sys_start_timer(p_scb->avrc_ct_timer, BTA_AV_RC_DISC_TIME_VAL_SPECIAL_DEVICE,
                            BTA_AV_AVRC_TIMER_EVT, p_scb->hndl);
        }
        else {
            bta_sys_start_timer(p_scb->avrc_ct_timer, BTA_AV_RC_DISC_TIME_VAL,
