...
typedef enum {
  ...
  // Some headsets will send discover request of AVDTP signaling as we did it in the same
  // time. And the discover response will be returned over 3.5s so that we will take it
  // as time-out. This workaround just make the timer longer then 3.5s.
  INTEROP_DISCOVER_TIME_OUT
