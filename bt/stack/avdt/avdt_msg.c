        ...
        if (curr_msg_len == 0)
        {
            ...
            if (msg == AVDT_MSG_TYPE_CMD)
            {
                    ...
                    (sig == AVDT_SIG_SECURITY) || (avdt_cb.rcb.ret_tout == 0))
                {
                    ...
                    if (interop_match_addr(INTEROP_DISCOVER_TIME_OUT, (const bt_bdaddr_t *)&p_ccb->peer_addr) == TRUE) {
                        AVDT_TRACE_DEBUG("Special device, last timer longer for AVDT response");
                        interval_ms = avdt_cb.rcb.sig_tout * 1000 * 10;
                    }
