...
static void transmit_downward(data_dispatcher_type_t type, void *data) {
  if (type == MSG_STACK_TO_HC_HCI_CMD) {
    ...
    // M:   multithread timing issue. when jni_workqueue thread shutdown, it free packet queue and close uart port.
    //      sometimes, workqueue thread receive controller event before close finished.
...
//M: host trigger FW assert 
/* Host trigger firmware assert in order to catch firmware core dump for analysis*/
#ifdef MTK_BT_COMMON
