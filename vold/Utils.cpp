...
status_t CreateDeviceNode(const std::string& path, dev_t dev) {
    ...
    if (sehandle) {
		LOG(ERROR) << "CreateDeviceNode sehandle is true";// add linqihan@wisky
        if (!selabel_lookup(sehandle, &secontext, cpath, S_IFBLK)) {
			LOG(ERROR) << "CreateDeviceNode selabel_lookup is false";// add linqihan@wisky
    ...
    if (secontext) {
		LOG(ERROR) << "CreateDeviceNode secontext exists";// add linqihan@wisky
