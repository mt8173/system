...
#include <sys/types.h>
#include <sys/wait.h>
// add by linqihan@wisky, for ntfs
#include "fs/Ntfs.h"
#include "fs/Exfat.h"
#include "fs/Ntfs.h"
// add end
...
PublicVolume::PublicVolume(dev_t device) :
    ...
    setId(StringPrintf("%u:%u", major(device), minor(device)));// modify by linqihan@wisky
...
status_t PublicVolume::doCreate() {
	LOG(ERROR) << "PublicVolume doCreate starts";
...
status_t PublicVolume::doMount() {
    ...
    readMetadata();
	if (mFsType != "vfat" && mFsType != "exfat" && mFsType != "ntfs") {// modify by linqihan@wisky,for NTFS & ExFat
    ...
    if (!(getMountFlags() & MountFlags::kVisible)) {
		...
		LOG(ERROR) << "kVisible is false";// add by linqihan@wisky
        return OK;
    }
...
status_t PublicVolume::doUnmount() {
	...
	ForceUnmount(getPath());// add by linqihan@wisky
