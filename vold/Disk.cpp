...
status_t Disk::create() {
    CHECK(!mCreated);
    mCreated = true;
    notifyEvent(ResponseCode::DiskCreated, StringPrintf("%d", mFlags |= 0x01));// modify by linqihan@wisky
    readMetadata();
    readPartitions();
    return OK;
}
...
status_t Disk::readPartitions() {
    ...
    for (auto line : output) {
        ...
        if (!strcmp(token, "DISK")) {
            ...
            if (table == Table::kMbr) {
                const char* type = strtok(nullptr, kSgdiskToken);
				LOG(ERROR) << "readPartitions disk type is " << type;// add linqihan@wisky
                switch (strtol(type, nullptr, 16)) {
                case 0x06: // FAT16
				case 0x07: // add by linqihan@wisky, for NTFS or ExFat
